...
function timeoutAbort(ms: number) { const c = new AbortController(); const t = setTimeout(() => c.abort(`timeout ${ms}ms`), ms); return { signal: c.signal, cancel: () => clearTimeout(t) }; }
async function postJSON(url: string, body: unknown, ms: number) {
  const { signal, cancel } = timeoutAbort(ms);
  try {
    const r = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body), signal });
    const text = await r.text(); let data: any = {}; try { data = text ? JSON.parse(text) : {}; } catch {} return { r, data, text };
  } finally { cancel(); }
}

meetRouter.post('/meet/webhook', async (req, res) => {
  const parsed = CreateMeetInput.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ ok:false, type:'zod', issues: parsed.error.issues });
  ...
  for (let attempt=0; attempt<=retries; attempt++) {
    try {
      ...
      if (!r.ok) return res.status(r.status).json({ ok:false, upstream:true, body: text || data });
      const link = data?.meetLink || data?.link || data?.url;
      if (!link || typeof link !== 'string') return res.status(502).json({ ok:false, error:'No meetLink from upstream', body:data });
      return res.json({ ok:true, meetLink: link });
    } catch (e:any) { lastErr = e; if (attempt === retries) break; }
  }
